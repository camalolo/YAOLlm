...
namespace Gemini
{
    public class SQLiteMemoryStore : IDisposable
    {
        ...
        public long StoreMemory(string content, float[] embedding, string? url = null)
        {
            ...
            if (embedding == null || embedding.Length != Utils.ExpectedEmbeddingDimension)
                throw new ArgumentException($"Embedding must be {Utils.ExpectedEmbeddingDimension} dimensions", nameof(embedding));
        ...
        private List<(long id, string content, float score, DateTime createdAt)> FetchAllMemories(float[] queryEmbedding)
        {
            ...
            using var command = _connection.CreateCommand();
            command.CommandText = "SELECT id, content, embedding, created_at FROM memory_content ORDER BY created_at DESC LIMIT 1000";

            using var reader = command.ExecuteReader();
            while (reader.Read())
            {
                long id = reader.GetInt64(0);
                string content = $"Memory #{id} :\n\n" + reader.GetString(1);
                byte[] embeddingBytes = (byte[])reader.GetValue(2);
                float[] embedding = new float[Utils.ExpectedEmbeddingDimension];
                Buffer.BlockCopy(embeddingBytes, 0, embedding, 0, embeddingBytes.Length);
                float score = Utils.CosineSimilarity(embedding, queryEmbedding);
