...
namespace Gemini
{
    public static class ApiFunctions
    {
        public static async Task<string?> SendToLLM(GeminiClient client, string prompt, string? imageBase64 = null)
        {
            ...
            // Instead of reassigning, modify the list in-place
            ...
            while (true)
            {
                try
                {
                    ...
                    if (response.StatusCode == System.Net.HttpStatusCode.BadRequest)
                    {
                        ...
                        client.Logger.Log($"Request sent : {payload}");
                ...
                {
                    client.Logger.Log($"Error in SendToLLM: {ex.Message}");
        ...
        public static async Task<string?> SendToLLMWithNoContext(GeminiClient client, string prompt)
        {
            try
            {
                ...
                var jsonPayload = JsonSerializer.Serialize(payload);
                client.Logger.Log($"Sending payload to LLM in SendToLLMWithNoContext: {jsonPayload}");
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.BadRequest)
                {
                    client.Logger.Log($"BadRequest (400) received in SendToLLMWithNoContext. Response content: {response.Content}");
                    client.Logger.Log($"Request payload: {jsonPayload}");
                    return null;
                }

                response.ThrowIfError();
                client.Logger.Log($"Raw LLM response in SendToLLMWithNoContext: {response.Content}");
            ...
            {
                client.Logger.Log($"Error in SendToLLMWithNoContext: {ex.Message}");
                return null;
            }
            finally
            {
